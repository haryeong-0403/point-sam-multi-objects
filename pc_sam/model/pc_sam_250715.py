...
class PointCloudSAM(nn.Module):
    ...
    def inference(
        ...
        coords: torch.Tensor,
        colors: torch.Tensor,
        prompt_coords: torch.Tensor = None,
        prompt_labels: torch.Tensor = None,
        multimask_output: bool = True,
    ):
        """
        Zero-shot inference using prompt coordinates (e.g. FPS).
        If prompt_coords is None, uses legacy dummy inference mode.
        """
        B, N, _ = coords.shape
        ...
        # normalize colors (if not already normalized)
        if colors.max() > 1.0:
            colors = colors / 255.0

        if prompt_coords is None:
            # legacy dummy mask inference
        ...
        # zero-shot inference using given prompts
        if prompt_labels is None:
            # assume all foreground prompts (label = 1)
            num_pts = prompt_coords.shape[1]
            prompt_labels = torch.ones((B, num_pts), dtype=torch.long, device=device)

        # run single mask prediction
        ...
        return [{"masks": masks, "iou_preds": iou_preds}]
